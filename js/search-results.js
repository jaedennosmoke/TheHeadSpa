/* ─── The Head Spa — Shopify Search Results v5.0 ─── */
(function() {
  'use strict';
  ...
  var state = {
    ...
    tags: [],
    allTags: {},
    variantOptions: {},      // { "Size": ["8oz","16oz"], ... } — selected
    allVariantOptions: {},   // { "Size": { "8oz": 3, "16oz": 5 }, ... } — collected
    cursor: null,
    hasNextPage: false,
    totalCount: 0,
    filterSections: {
      ...
      productType: false,
      tags: false,
      variantOptions: {},    // keyed by option name
    },
    brandSearch: '',
  };
  ...
  /* ─── GraphQL ─── */
  function buildGQL() {
    return 'query search($query:String!,$first:Int!,$after:String,$sortKey:SearchSortKeys!,$reverse:Boolean,$filters:[ProductFilter!])' +
      '{search(query:$query,first:$first,after:$after,types:[PRODUCT],sortKey:$sortKey,reverse:$reverse,productFilters:$filters)' +
      '{totalCount pageInfo{hasNextPage endCursor}edges{node{... on Product{id title handle vendor productType tags availableForSale ' +
      'options{name values}' +
  ...
  /* ─── Collect filter data from results ─── */
    ...
    products.forEach(function(p) {
      ...
      if (p.tags && p.tags.length) {
        p.tags.forEach(function(t) {
          state.allTags[t] = (state.allTags[t] || 0) + 1;
        });
      }
      if (p.options && p.options.length) {
        p.options.forEach(function(opt) {
          if (opt.name === 'Title') return; // skip default "Title" option
  ...
  function renderSidebar() {
    ...
    var availBody = '<label class="ths-sr-cb-label">' +
      '<input type="checkbox" class="ths-sr-cb" id="ths-sr-instock"' + (state.inStockOnly ? ' checked' : '') + '>' +
    ...
    var priceBody = '<div class="ths-sr-price-row">' +
      '<div class="ths-sr-price-field"><span class="ths-sr-price-label">$</span>' +
        '<input type="number" id="ths-sr-pmin" class="ths-sr-price-input" placeholder="Min" min="0" step="1"' + (state.priceMin !== null ? ' value="' + state.priceMin + '"' : '') + '></div>' +
      '<span class="ths-sr-price-to">to</span>' +
      '<div class="ths-sr-price-field"><span class="ths-sr-price-label">$</span>' +
        '<input type="number" id="ths-sr-pmax" class="ths-sr-price-input" placeholder="Max" min="0" step="1"' + (state.priceMax !== null ? ' value="' + state.priceMax + '"' : '') + '></div>' +
      '<button id="ths-sr-price-go" class="ths-sr-price-go" aria-label="Apply price filter"><svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M9 18l6-6-6-6" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg></button>' +
    '</div>';
    ...
    var brandBody = (vendors.length > 5
      ? '<div class="ths-sr-brand-search-wrap">' +
          ...
          '<input type="text" id="ths-sr-brand-search" class="ths-sr-brand-search" placeholder="Search Brand" value="' + esc(state.brandSearch) + '">' +
        '</div>' : '') +
    ...
    var typeBody = '<div class="ths-sr-scrollable-list">' +
      buildCheckboxList(productTypes, state.allProductTypes, state.productTypes, 'ptype') +
    '</div>';

    /* Tags */
    var tagsBody = '<div class="ths-sr-scrollable-list">' +
      buildCheckboxList(tagsList, state.allTags, state.tags, 'tag') +
    '</div>';

    /* Variant Options — one section per option name */
    var variantSections = '';
    ...
    return '' +
      '<aside class="ths-sr-sidebar">' +
        '<h1 class="ths-sr-sidebar-heading">Search Results</h1>' +
        '<p class="ths-sr-sidebar-count">' + state.totalCount + ' result' + (state.totalCount !== 1 ? 's' : '') + ' for "' + esc(state.query) + '"</p>' +
  ...
  function renderSortBar() {
    return '<div class="ths-sr-sort-bar">' +
      '<span class="ths-sr-result-text">' + state.totalCount + ' result' + (state.totalCount !== 1 ? 's' : '') + '</span>' +
      '<div class="ths-sr-sort-wrap">' +
        '<label class="ths-sr-sort-label">Sort by:</label>' +
        '<select id="ths-sr-sort" class="ths-sr-sort-select">' +
          '<option value="RELEVANCE"' + (state.sortKey === 'RELEVANCE' ? ' selected' : '') + '>Relevance</option>' +
  ...
  function renderProduct(product) {
    ...
    return '' +
      '<div class="ths-card" data-handle="' + product.handle + '">' +
  ...
  function renderPage(el, products) {
    ...
    el.innerHTML = '<div class="ths-sr-layout">' +
      renderSidebar() +
      '<div class="ths-sr-main">' + renderSortBar() + grid + '</div>' +
    '</div>';
  ...
  function bindAllEvents(container) {
    ...
    container.querySelectorAll('.ths-sr-section-toggle').forEach(function(btn) {
      btn.addEventListener('click', function() {
        ...
        if (voName) {
          state.filterSections.variantOptions[voName] = !state.filterSections.variantOptions[voName];
        } else {
    ...
    /* Sort */
    var sortEl = container.querySelector('#ths-sr-sort');
    if (sortEl) sortEl.addEventListener('change', function() {
      var v = sortEl.value;
      if (v === 'RELEVANCE') { state.sortKey = 'RELEVANCE'; state.reverse = false; }
    ...
    /* In Stock */
    var stockEl = container.querySelector('#ths-sr-instock');
    ...
    /* Price */
    var priceGo = container.querySelector('#ths-sr-price-go');
    if (priceGo) priceGo.addEventListener('click', function() {
      var mn = container.querySelector('#ths-sr-pmin');
      var mx = container.querySelector('#ths-sr-pmax');
      state.priceMin = mn.value ? parseFloat(mn.value) : null;
      state.priceMax = mx.value ? parseFloat(mx.value) : null;
      doSearch();
    });
    container.querySelectorAll('#ths-sr-pmin, #ths-sr-pmax').forEach(function(inp) {
      inp.addEventListener('keydown', function(e) { if (e.key === 'Enter') { e.preventDefault(); priceGo.click(); } });
    });

    /* Brand search */
    var brandSearchEl = container.querySelector('#ths-sr-brand-search');
  ...
  /* Generic checkbox group binding */
  function bindCheckboxGroup(container, dataAttr, stateArray, callback) {
    container.querySelectorAll('.ths-sr-cb[data-' + dataAttr + ']').forEach(function(cb) {
      cb.addEventListener('change', function() {
        ...
        if (cb.checked) {
          if (stateArray.indexOf(v) === -1) stateArray.push(v);
        } else {
          var idx = stateArray.indexOf(v);
  ...
  function bindCardEvents(container) {
    container.querySelectorAll('.ths-card').forEach(function(card) {
      ...
      overlay.addEventListener('click', function(e) { e.preventDefault(); e.stopPropagation(); window.location.href = '/products/' + card.getAttribute('data-handle'); });
    });
    container.querySelectorAll('.ths-card__add-bag:not([disabled])').forEach(function(btn) {
      btn.addEventListener('click', function(e) { e.preventDefault(); e.stopPropagation(); var card = btn.closest('.ths-card'); if (card) window.location.href = '/products/' + card.getAttribute('data-handle'); });
    });
  }

  /* ─── Search ─── */
  async function doSearch(append) {
    var el = document.getElementById('ths-search-results');
    if (!el) return;

    if (!append) {
      var gridEl = el.querySelector('.ths-sr-grid');
      var lm = el.querySelector('.ths-sr-load-more-wrap');
      var counts = el.querySelectorAll('.ths-sr-result-text, .ths-sr-sidebar-count');
      if (lm) lm.remove();
      if (gridEl) gridEl.innerHTML = '<div class="ths-sr-loading-inline"><div class="ths-sr-spinner"></div></div>';
      counts.forEach(function(c) { c.textContent = 'Searching...'; });
    }

    var variables = {
      query: state.query, first: CONFIG.resultsPerPage,
    ...
    try {
      ...
      var products = data.search.edges.map(function(e) { return e.node; });
      state.totalCount = data.search.totalCount;
      state.cursor = data.search.pageInfo.endCursor;
  ...
  async function init() {
    var el = document.getElementById('ths-search-results');
    if (!el) return;
    state.query = getSearchTerm();
    if (!state.query) {
      el.innerHTML = '<div class="ths-sr-empty"><p>Enter a search term to find products.</p><p><a href="/shop">Browse all products</a></p></div>';
      return;
    }
    el.innerHTML = '<div class="ths-sr-loading"><div class="ths-sr-spinner"></div><p>Searching products...</p></div>';

    try {
      var data = await api(buildGQL(), { query: state.query, first: CONFIG.resultsPerPage, after: null, sortKey: 'RELEVANCE', reverse: false });
      var products = data.search.edges.map(function(e) { return e.node; });
      state.totalCount = data.search.totalCount;
      state.cursor = data.search.pageInfo.endCursor;
      state.hasNextPage = data.search.pageInfo.hasNextPage;
  ...
  function interceptSearch() {
    document.querySelectorAll('form.e-search-form').forEach(function(form) {
      form.addEventListener('submit', function(e) {
        e.preventDefault();
        var input = form.querySelector('input[name="s"]');
        if (input && input.value.trim()) window.location.href = '/search-results/?q=' + encodeURIComponent(input.value.trim());
      });
    });
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() { interceptSearch(); init(); });
  } else { interceptSearch(); init(); }
})();